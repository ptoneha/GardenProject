  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['agriculture:land:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="landList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['agriculture:land:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['agriculture:land:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const data = reactive({
  ...
  rules: {
    ...
    inputValue: [
      { required: true, message: "用户输入数值不能为空", trigger: "blur" }
    ],
    unit: [
      { required: true, message: "输入单位不能为空", trigger: "change" }
    ],
    lightLevel: [
      { required: true, message: "光照等级不能为空", trigger: "change" }
    ],
  }
})
...
function handleUpdate(row) {
  reset()
  const _landId = row.landId || ids.value
  getLand(_landId).then(response => {
  ...
  proxy.$refs["landRef"].validate(valid => {
    if (valid) {
      if (form.value.landId != null) {
        updateLand(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addLand(form.value).then(response => {
...
function handleDelete(row) {
  const _landIds = row.landId || ids.value
  proxy.$modal.confirm('是否确认删除地块资源管理编号为"' + _landIds + '"的数据项？').then(function() {
    return delLand(_landIds)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('agriculture/land/export', {
    ...queryParams.value
